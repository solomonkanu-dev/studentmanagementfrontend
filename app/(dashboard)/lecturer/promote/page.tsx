...
export default function LecturerPromotePage() {
  ...
  // Lecturer's own classes (source options)
  const { data: myClasses = [], isLoading: myClassesLoading } = useQuery({
    queryKey: ["lecturer-my-classes"],
    queryFn: lecturerApi.getMyClasses,
  });

  // All institute classes (target options)
  const { data: allClasses = [] } = useQuery({
    queryKey: ["institute-classes"],
    queryFn: lecturerApi.getAllInstituteClasses,
  });

  const { data: eligibility = [], isLoading: eligibilityLoading } = useQuery({
    queryKey: ["lecturer-promotion-eligibility", sourceClassId, gradeThreshold, attendanceThreshold],
    queryFn: () =>
      lecturerApi.getPromotionEligibility(sourceClassId, { gradeThreshold, attendanceThreshold }),
    enabled: !!sourceClassId && step === "review",
  });

  const promoteMutation = useMutation({
    mutationFn: lecturerApi.bulkPromote,
    onSuccess: (data) => {
  ...
  const typedMyClasses = myClasses as LecturerClass[];
  const typedAllClasses = allClasses as unknown as Class[];
  const typedEligibility = eligibility as PromotionEligibilityEntry[];

  // Target options: all classes except the source
  const targetOptions = typedAllClasses.filter((c) => c._id !== sourceClassId);
  ...
  function handleConfirm() {
    promoteMutation.mutate({ sourceClassId, targetClassId, studentIds: included.map((s) => s._id) });
  }
  ...
  const sourceClass = typedMyClasses.find((c) => c._id === sourceClassId);
  const targetClass = typedAllClasses.find((c) => c._id === targetClassId);

  // Empty state: lecturer has no classes
  if (!myClassesLoading && typedMyClasses.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        <School className="h-10 w-10 text-body" />
        <p className="text-sm font-medium text-black dark:text-white">No classes assigned</p>
        <p className="text-xs text-body">You need to be assigned to a class before you can promote students.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-meta-5/10">
          <GraduationCap className="h-5 w-5 text-meta-5" />
        </div>
        <div>
          <h1 className="text-lg font-bold text-black dark:text-white">Promote Students</h1>
          <p className="text-sm text-body">Review eligibility and promote your class to the next level</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            <div className="grid gap-5 sm:grid-cols-2">
              {/* Source — only lecturer's own classes */}
              <div>
            ...
            {sourceClassId && targetClassId && (
              <div className="flex items-center gap-3 rounded-xl border border-meta-5/30 bg-meta-5/5 px-4 py-3 text-sm">
                <span className="font-semibold text-black dark:text-white">{sourceClass?.name}</span>
                <ArrowRight className="h-4 w-4 text-meta-5" />
            ...
            <div className="flex justify-end">
              <Button
                ...
                className="bg-meta-5 hover:bg-meta-5/90"
              >
                Check Eligibility
              </Button>
            </div>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setExcluded(new Set())} className="text-xs font-medium text-primary hover:underline">
                    Select all
                  </button>
                  <button
                  ...
                  <button
                    onClick={() => setExcluded(new Set(typedEligibility.filter((s) => !s.clearForPromotion).map((s) => s._id)))}
            ...
            <CardContent className="p-0">
              {eligibilityLoading ? (
                <div className="flex h-32 items-center justify-center">
                  <div className="h-5 w-5 animate-spin rounded-full border-2 border-stroke border-t-meta-5" />
                </div>
                    ...
                    return (
                      <label
                        ...
                        className={[
                          ...
                          !isExcluded && !student.clearForPromotion ? "bg-yellow-50/60 dark:bg-yellow-400/5" : "",
                      ...
                      >
                        <input
                          ...
                          className="mt-0.5 h-4 w-4 shrink-0 rounded border-stroke accent-meta-5"
                        />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {isExcluded && <Badge variant="danger">Excluded</Badge>}
                          </div>
          ...
          <CardContent className="flex flex-col items-center gap-4 py-10">
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-sm text-body">
                {result.promoted} student{result.promoted !== 1 ? "s" : ""} promoted from{" "}
            ...
            <div className="flex items-center gap-2 text-xs text-body">
              ...
              Historical records remain linked to the original class.
            </div>
            <Button onClick={reset} className="bg-meta-5 hover:bg-meta-5/90">Promote Another Class</Button>
          </CardContent>
        </Card>
      )}
    </div>
